...
printf '>>> Training config: max_steps=%s max_seq_length=%s load_in_4bit=%s cache_processed_samples=%s dataloader_num_workers=%s lora_dropout=%s disable_unsloth_compile=%s\n' \
  "${MAX_STEPS}" "${MAX_SEQ_LENGTH}" "${LOAD_IN_4BIT}" "${CACHE_PROCESSED_SAMPLES}" "${DATALOADER_NUM_WORKERS}" "${LORA_DROPOUT}" "${DISABLE_UNSLOTH_COMPILE}"
"${TRAIN_CMD[@]}"
